...
# Dart formatter commonly splits client.rpc(...) across multiple lines. Match
# semantically rather than requiring the method name and RPC literal to share
# one physical line.
uses_content_detail_rpc = bool(
    re.search(r"client\s*\.\s*rpc\s*\(\s*['\"]get_content_detail['\"]", r, re.DOTALL)
)
if not uses_content_detail_rpc:
    errors.append('repository_not_using_rpc')

result = {
    'ok': not errors,
    'passed_checks': 12 - len(errors),
    'expected_checks': 12,
    'errors': errors,
}
